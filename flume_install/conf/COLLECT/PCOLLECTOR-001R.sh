...
h=PCOLLECTOR-001R.conf
cd $BASE_HOME

for i in {1..10}
do
sector=`awk '{if ($13~/'"$i"'\./ &&$5=="EDGEHSM") print $1}' COLLECT_PORT.INFO`
port3=`awk '{if ($11~/'"$ipaddr"'/ && $4=="picture" && $13~/'"$i"'\./) print $12}' COLLECT_PORT.INFO`
type3=`awk '{if ($11~/'"$ipaddr"'/ && $4=="picture" && $13~/'"$i"'\./) print $5}' COLLECT_PORT.INFO`
device3=`awk '{if ($11~/'"$ipaddr"'/ && $4=="picture" && $13~/'"$i"'\./) print $6}' COLLECT_PORT.INFO`
agent3=`awk '{if ($11~/'"$ipaddr"'/ && $4=="picture" && $13~/'"$i"'\./) print $2}' COLLECT_PORT.INFO`
...
agent1.sources.avro-source$i.port = $port3
...
agent1.sinks.hdfs-sink$i.agent.id = $agent3
...
agent1.sinks.hdfs-sink$i.business.type = picture
agent1.sinks.hdfs-sink$i.log.type = $type3
agent1.sinks.hdfs-sink$i.device.id = $device3" >>$h
echo "" >>$h
done
