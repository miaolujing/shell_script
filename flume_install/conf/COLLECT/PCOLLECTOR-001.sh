...
h=PCOLLECTOR-001.conf
cd $BASE_HOME

for i in {1..10}
do
sector=`awk '{if ($13~/'"$i"'\./ &&$5=="EDGEHSM") print $1}' COLLECT_PORT.INFO`
port1=`awk '{if ($8~/'"$ipaddr"'/ && $4=="picture" && $13~/'"$i"'\./) print $9}' COLLECT_PORT.INFO`
type1=`awk '{if ($8~/'"$ipaddr"'/ && $4=="picture" && $13~/'"$i"'\./) print $5}' COLLECT_PORT.INFO`
device1=`awk '{if ($8~/'"$ipaddr"'/ && $4=="picture" && $13~/'"$i"'\./) print $6}' COLLECT_PORT.INFO`
agent1=`awk '{if ($8~/'"$ipaddr"'/ && $4=="picture" && $13~/'"$i"'\./) print $2}' COLLECT_PORT.INFO`
...
agent1.sources.avro-source$i.port = $port1
...
agent1.sinks.hdfs-sink$i.agent.id = $agent1
...
agent1.sinks.hdfs-sink$i.log.type = $type1
agent1.sinks.hdfs-sink$i.device.id = $device1" >>$h
echo "" >>$h
done
