...
h=VCOLLECTOR-001R.conf
cd $BASE_HOME

for i in {1..10}
do
sector=`awk '{if ($13~/'"$i"'\./ &&$5=="EDGEHSM") print $1}' COLLECT_PORT.INFO`
port4=`awk '{if ($11~/'"$ipaddr"'/ && $4=="video" && $13~/'"$i"'\./) print $12}' COLLECT_PORT.INFO`
type4=`awk '{if ($11~/'"$ipaddr"'/ && $4=="video" && $13~/'"$i"'\./) print $5}' COLLECT_PORT.INFO`
device4=`awk '{if ($11~/'"$ipaddr"'/ && $4=="video" && $13~/'"$i"'\./) print $6}' COLLECT_PORT.INFO`
agent4=`awk '{if ($11~/'"$ipaddr"'/ && $4=="video" && $13~/'"$i"'\./) print $2}' COLLECT_PORT.INFO`
...
agent1.sources.avro-source$i.port = $port4
...
agent1.sinks.hdfs-sink$i.agent.id = $agent4
...
agent1.sinks.hdfs-sink$i.log.type = $type4
agent1.sinks.hdfs-sink$i.device.id = $device4" >>$h
echo "" >>$h
done
